...
//DeltaRule class - this algorithm updates the weights according to cost function
// we can find a directions using the partial derivative of the cost function

public class DeltaRule extends LearningAlgorithm {

    public ArrayList<ArrayList<Double>> error; //matrix that stores errors for each output record comb
    public ArrayList<Double> generalError;
    public ArrayList<Double> overallError;
    public double overallGeneralError; //overall error for all outputs and records
    ...
    private int currentRecord=0; //index of the record being fed into the neural network
    private ArrayList<ArrayList<ArrayList<Double>>> newWeights; //cubic matrix used to update all new values of weights
        ...
        if(neuralNet.getNumberOfHiddenLayers()>0){
            ...
            switch(learningMode){
                case BATCH:
                    ...
                    while(epoch<MaxEpochs && overallGeneralError>MinOverallError){ //training stops wehn epoch reach maximum or error reaches min
                        epoch++;
